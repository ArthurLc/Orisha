...
public class AI_EnemyStateIsHitTutorial : AI_EnemyStateIsHit
{
    ...
    protected override void CurrentUpdate()
    {
        ...
        if (myIndividu.agentIsControlledByOther)
        {
            ...
            {
                (myIndividu as AI_Enemy_Tutorial).ChangeState(AI_Enemy_Tutorial.State.Fleeing);
            }
            else
            {
    ...
    public override void PropulseAgent(Vector3 _dir)
    {
        ...
        (myIndividu as AI_Enemy_Tutorial).ChangeState(AI_Enemy_Tutorial.State.IsHit, true);
    }
}
